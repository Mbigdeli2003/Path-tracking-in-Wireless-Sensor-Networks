...
hold on
plot(X_Hat(1,1:it),X_Hat(2,1:it),'y','linewidth',4);
hold on
plot(X_hat(1,1:it),X_hat(2,1:it),'r--','linewidth',2.5);legend('Real Tatrget','EKF Estimation','first estimation')
plot(n,m,'r*','linewidth',2.5)
hold on
plot(NN(:,:),MM(:,:),'g*');grid

ERROR_R_X=abs(X_Hat(1,1:it)-X);
ERROR_R_Y=abs(X_Hat(2,1:it)-y_path');
...
ERROR_X=abs(X-X_hat(1,1:it));
ERROR_Y=abs(y_path'-X_hat(2,1:it));
