...
%       clear P X_HAT inv_P  Q x_abs_1 x_abs_2 y_abs_1 y_abs_2
%       clear r_1 r_2 H_1 H_2 Z_1 Z_2 H_k Z_k delta_1 delta_2 delta
%        inv_P(:,:,1)=1e6*eye(4);
 ...
 varN=0.01^(1/2);
  r=(random('normal',0,varN,nn,2));
   r=r';
            ...
            Q=0.001^2*eye(2);
             X_HAT(:,j)=A*X_HAT(:,j-1)+B*Q*[0.01 0.01]';
             N_Best=N_CAND{i};
             M_Best=M_CAND{i};
             x_abs_1(j)=abs(X_HAT(1,j)-(N_Best(1)));
             x_abs_2(j)=abs(X_HAT(1,j)-(N_Best(2)));
             y_abs_1(j)=abs(X_HAT(2,j)-M_Best(1));
             y_abs_2(j)=abs(X_HAT(2,j)-M_Best(2));
             r_1(j)=(x_abs_1(j)^2+y_abs_1(j)^2)^(1/2);
             r_2(j)=((x_abs_2(j))^2+(y_abs_2(j)^2))^(1/2);
             
            H_1(j)=atan2((X_HAT(1,j)-(N_Best(1))),(X_HAT(2,j)-(M_Best(1))));
            H_2(j)=atan2((X_HAT(1,j)-N_Best(2)),(X_HAT(2,j)-M_Best(2)));
         ...
         R=0.01*eye(SIZE_DELTA(2));
         
         P(:,:,j)=(A*P(:,:,j-1)*A')+B*Q*B';

         inv_P(:,:,j+1)=inv(P(:,:,j))+delta*inv(R)*delta';
         P(:,:,j+1)=inv(inv_P(:,:,j+1));
         K=(P(:,:,j+1))*(delta)*inv(R);
          P(:,:,j)=P(:,:,j+1);  
%          
%          
           X_HAT(:,j+1)=X_HAT(:,j)+(K*(Z_k-H_k));
            X_HAT(:,j)=X_HAT(:,j+1);
           X_HAT_d(:,j)=X_HAT(:,j+1);
